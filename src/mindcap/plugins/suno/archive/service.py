...
from mindcap.core.models import CaptureEnvelope, CaptureRequest, CapturedAsset
    ...
    def _asset_candidates(self, clip: dict[str, Any], options: dict[str, Any]) -> list[tuple[str, str, str, str]]:
        ...
        if isinstance(audio_url, str) and audio_url:
            candidates.append(
                (
                    ...
                    audio_url,
                    "audio/mpeg",
                    f"clips/{clip_id}/audio/original.{options.get('audio_format', 'mp3')}",
                )
            )
        image_url = clip.get("image_url") or metadata.get("image_url")
        if isinstance(image_url, str) and image_url:
            candidates.append(
                ("artwork", image_url, "image/jpeg", f"clips/{clip_id}/artwork/cover.jpg")
            )
        ...
        if options.get("include_video", True):
            ...
            if isinstance(video_url, str) and video_url:
                candidates.append(
                    ("video", video_url, "video/mp4", f"clips/{clip_id}/video/video.mp4")
                )
        return candidates
    ...
    def capture(self, request: CaptureRequest) -> CaptureEnvelope:
        ...
        for clip_id, clip in sorted(clips_by_id.items()):
            ...
            if aligned is not None:
                _, record = aligned
                response_units.append(
                    record.to_raw_response_unit(f"aligned-lyrics-{clip_id}", len(response_units))
                )
            for asset_type, url, media_type, relative_path in self._asset_candidates(
                ...
                try:
                    ...
                    warnings.append(f"Asset download failed for {clip_id} ({asset_type}): {error}")
